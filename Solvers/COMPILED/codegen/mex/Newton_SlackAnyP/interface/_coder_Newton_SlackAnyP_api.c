 ...
 *
 * _coder_Newton_SlackAnyP_api.c
 *
 * Code generation for function '_coder_Newton_SlackAnyP_api'
 *
 */
  ...
  const emlrtMsgIdentifier *msgId))[9]
{
  ...
  void Newton_SlackAnyP_api(const mxArray * const prhs[16], int32_T nlhs, const
  mxArray *plhs[6])
{
  real_T (*f)[4];
  real_T (*s)[3];
  ...
  P = i_emlrt_marshallIn(&st, emlrtAliasP(prhs[14]), "P");
  alpha = i_emlrt_marshallIn(&st, emlrtAliasP(prhs[15]), "alpha");

  /* Invoke the target function */
  Newton_SlackAnyP(&st, *X0, *Aeq, *WTarget, *tmin, *tmax, tau1, tau2,
                   *OptimalTension, Tol, IterMax, *KappaMat, *Q, epsilon, SqFac,
                   P, alpha, *f, *s, *lambda, &N_Iter, &InfeasibleFlag,
                   &LineIter);
...
/* End of code generation (_coder_Newton_SlackAnyP_api.c) */
